...
class Action {
	...
	Action(int aActionId, const string& aName, bool aActive) 
		throw() : actionId(aActionId), name(aName), active(aActive) { };
	...
	struct Raw {
		...
		throw() : id(aId), rawId(aRawId), name(aName), raw(aRaw), time(aTime), active(aActive) { };
...
public:
	...
	int addAction(int actionId, const string& name, bool active) throw(Exception);
	...
	void addRaw(uint64_t time, UserPtr aUser, const string& aRaw) {
		Lock l(act);
...
private:
	...
	typedef map<uint64_t, pair<UserPtr, string>> ListRaw;
