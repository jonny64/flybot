...
void DownloadManager::addConnection(UserConnectionPtr conn) {
	if(!conn->isSet(UserConnection::FLAG_SUPPORTS_TTHF) || !conn->isSet(UserConnection::FLAG_SUPPORTS_ADCGET)) {
		...
		if(BOOLSETTING(NO_TTH_CHEAT))
			ClientManager::getInstance()->setCheating(conn->getUser(), "", "Old client - TTH not fully supported", SETTING(OLD_CLIENT_RAW), true);
	...
	if(d->getSize() == -1) {
		if(bytes >= 0) {
			...
			if(d->getType() == Transfer::TYPE_FULL_LIST) {
				ClientManager::getInstance()->setListSize(aSource->getUser(), bytes);
			}
		} else {
...
/**
 * @file
 * $Id: DownloadManager.cpp 386 2008-05-10 19:29:01Z BigMuscle $
 */
