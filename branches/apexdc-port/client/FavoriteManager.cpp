...
bool FavoriteManager::addFavoriteDir(const string& aDirectory, const string& aName, const string& aExt){
	...
	for(FavDirIter i = favoriteDirs.begin(); i != favoriteDirs.end(); ++i) {
		if((Util::strnicmp(path, i->dir, i->dir.length()) == 0) && (Util::strnicmp(path, i->dir, path.length()) == 0)) {
			return false;
		}
		if(Util::stricmp(aName, i->name) == 0) {
			return false;
		}
		if(!aExt.empty() && Util::stricmp(aExt, i->ext) == 0) {
			return false;
		}
	}
...
bool FavoriteManager::removeFavoriteDir(const string& aName) {
	...
	for(FavDirIter j = favoriteDirs.begin(); j != favoriteDirs.end(); ++j) {
		if(Util::stricmp(j->dir.c_str(), d.c_str()) == 0) {
...
bool FavoriteManager::renameFavoriteDir(const string& aName, const string& anotherName) {
	...
	for(FavDirIter j = favoriteDirs.begin(); j != favoriteDirs.end(); ++j) {
		if(Util::stricmp(j->name.c_str(), aName.c_str()) == 0) {
			j->name = anotherName;
			save();
			return true;
		}
...
void FavoriteManager::save() {
	...
	try {
		...
		for(FavDirIter i = spl.begin(); i != spl.end(); ++i) {
...
void FavoriteManager::load() {
	...
	// Add ADC op commands (adchpp, assuming it'll become standard)
	static const char dscstr[] = "HDSC %[userSID]\n";
	addUserCommand(UserCommand::TYPE_RAW_ONCE, UserCommand::CONTEXT_CHAT | UserCommand::CONTEXT_SEARCH, UserCommand::FLAG_NOSAVE, 
		STRING(KICK_USER), dscstr, "adc://op");
...
void FavoriteManager::load(SimpleXML& aXml) {
	...
	// If there is nothing to load (new install) insert our support hub
	if(!Util::fileExists(getConfigFile())) {
		...
		e->setServer("techgeeks.geekgalaxy.com:1411");
...
/**
 * @file
 * $Id: FavoriteManager.cpp 386 2008-05-10 19:29:01Z BigMuscle $
 */
