...
void AdcHub::handle(AdcCommand::INF, AdcCommand& c) throw() {
	...
	if(u->getUser() == getMyIdentity().getUser()) {
		if(getMyIdentity().getSID() == 0 && u->getIdentity().isOp()) {
	...
	if(u->getIdentity().isHub()) {
		if(BOOLSETTING(STRIP_TOPIC)) {
			u->getIdentity().setDescription(Util::emptyString);
		} else if(BOOLSETTING(USE_FAV_NAMES)) {
			if(!getLocalHubName().empty())
				u->getIdentity().setNick(getLocalHubName());
			if(!getLocalHubDescription().empty())
				u->getIdentity().setDescription(getLocalHubDescription());
		}
...
void AdcHub::handle(AdcCommand::MSG, AdcCommand& c) throw() {
	...
	if(c.getParam("PM", 1, pmFrom)) { // add PM<group-cid> as well
		...
		if(from->getUser() != getMyIdentity().getUser())
			PluginManager::getInstance()->onIncomingPM(replyTo, message);

		fire(ClientListener::PrivateMessage(), this, *from, *to, *replyTo, message, c.hasFlag("ME", 1));
	} else {
		if(from->getUser() != getMyIdentity().getUser())
			PluginManager::getInstance()->onIncomingChat(this, message);

		fire(ClientListener::Message(), this, *from, message, c.hasFlag("ME", 1));
	}
}
...
void AdcHub::info(bool /*alwaysSend*/) {
	...
	addParam(lastInfoMap, c, "VE", getStealth() ? ("++ " DCVERSIONSTRING) : ("ApexDC++ " VERSIONSTRING));
