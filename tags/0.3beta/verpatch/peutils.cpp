...
bool clearPdbPath( PLOADED_IMAGE pim )
{
	...
	if ( !pdbname || (pdbname - pdd) <= 3 || strchr( pdd, '\\' ) == pdbname ) {
		d2print("PDB path too short, not changing\n");
		ret = true; __leave;
	}

	memcpy( pdd, "d:", 2 );
	strcpy_s( pdd + 2, dsize2 - 2, pdbname );
	char *t = pdd + strlen(pdd);
	ZeroMemory( t, dsize2 - (t - pdd) );
...
PSTR strFilePathDeUnicode( __in PCWSTR tstr)
{
	...
	for (int i = 0; i < MAX_PATH; i++ ) {
		...
		p[i] = 0;
		reinterpret_cast<PUCHAR>(p)[i] = w & 0xFF;
	}
